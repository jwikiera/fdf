...
void	draw_map(t_data *data, t_map_struct *map_struct, void *mlx, void *mlx_win)
{
	...
	free(data->screen_info->rotation_center);
	...
	for (int i = 0; i < map_struct->size_y; i ++) {
		for (int j = 0; j < map_struct->size_x; j ++) {
			...
			{
				t_vec3d	*proj1 = project_orthogonal(previous_point_y, data->screen_info->screen_plane, data->screen_info);
				t_vec3d *proj2 = project_orthogonal(pt, data->screen_info->screen_plane, data->screen_info);
				if (proj1 && proj2)
					connect_vects(data, proj1, proj2, 0x00000000);
				free_vectors(proj1, proj2, 0, 0);
			}
			else if (previous_point_y && data->screen_info->projection == perspective)
			{
				t_vec3d	*proj1 = project_perspective(previous_point_y, data->screen_info->screen_plane, data->screen_info);
				t_vec3d *proj2 = project_perspective(pt, data->screen_info->screen_plane, data->screen_info);
				if (proj1 && proj2)
					connect_vects(data, proj1, proj2, 0x00000000);
				free_vectors(proj1, proj2, 0, 0);
			}
	...
	for (int i = 0; i < map_struct->size_x; ++i) {
		for (int j = 0; j < map_struct->size_y; ++j) {
			...
			{
				t_vec3d	*proj1 = project_orthogonal(previous_point_x, data->screen_info->screen_plane, data->screen_info);
				t_vec3d *proj2 = project_orthogonal(pt, data->screen_info->screen_plane, data->screen_info);
				if (proj1 && proj2)
					connect_vects(data, proj1, proj2, 0x00000000);
				free_vectors(proj1, proj2, 0, 0);
			...
			{
				t_vec3d	*proj1 = project_perspective(previous_point_x, data->screen_info->screen_plane, data->screen_info);
				t_vec3d *proj2 = project_perspective(pt, data->screen_info->screen_plane, data->screen_info);
				if (proj1 && proj2)
					connect_vects(data, proj1, proj2, 0x00000000);
				free_vectors(proj1, proj2, 0, 0);
...
int destroy(t_vars *vars)
{
	...
	//ft_printf("final destroy\n");
	//free(vars);
	mlx_destroy_window(vars->mlx, vars->win);
	ft_printf("on destroy exit\n");
	exit(0);
	//return (1);
}
...
int main(int argc, char *argv[])
{
	...
	draw_map(data, vars->map_struct, mlx, mlx_win);
